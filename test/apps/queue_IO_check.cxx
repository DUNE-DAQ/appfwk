...
main(int argc, char* argv[])
{
  ...
  desc.add_options()("queue_type",
    ...
    "nelements", bpo::value<int>(), num_elements_desc.str().c_str())(
  ...
  if (vm.count("pause_between_pushes")) {
    ...
    if (avg_milliseconds_between_pushes < 0) {
      throw dunedaq::appfwk::ParameterDomainIssue(ERS_HERE,
  ...
  if (vm.count("initial_capacity_used")) {
    ...
    if (initial_capacity_used < 0 || initial_capacity_used > 1) {
      throw dunedaq::appfwk::ParameterDomainIssue(
        ERS_HERE, "Initial fractional capacity of queue which is used must lie in the range [0, 1]");
    }
  }
  ...
  TLOG(TLVL_INFO) << "Total time from start of thread launch to the last thread wrapping up was " << total_time
                  << " ms";
