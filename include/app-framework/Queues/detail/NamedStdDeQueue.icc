...
template <class T>
NamedStdDeQueue<T>::NamedStdDeQueue(const std::string& name, size_t size) : NamedQueueI<T>(name), fMaxSize(size), fDeque(), fSize(0) {}

template <class T>
void NamedStdDeQueue<T>::push(value_type &&object_to_push,
  ...
  if ( this->can_push() ) {
    ...
    errmsg
        << "In NamedStdDeQueue::push: unable to push since queue is full ("
...
template <class T>
T NamedStdDeQueue<T>::pop(const duration_type &timeout) {
  ...
  if (this->can_pop()) {
    ...
    std::stringstream errmsg;
    errmsg << "In NamedStdDeQueue::pop_wait_for: unable to pop since queue is "
...
template <class T>
void NamedStdDeQueue<T>::try_lock_for(std::unique_lock<std::mutex> &lk,
                                     const duration_type &timeout) {
  assert(!lk.owns_lock());
  ...
  if (!lk.owns_lock()) {
    ...
    errmsg << "Unable to lock the NamedStdDeQueue's mutex "
