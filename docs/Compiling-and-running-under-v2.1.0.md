...
```bash
git clone https://github.com/DUNE-DAQ/daq-buildtools.git -b dunedaq-v2.1.0
```
...
```sh
dbt-create.sh <release>  # dunedaq-v2.1.0 is the most recent release as of Jan-15-2021
```
...
```
cd sourcecode
git clone https://github.com/DUNE-DAQ/listrev.git -b dunedaq-v2.1.0
cd ..
```
Note the assumption above is that you aren't developing listrev; if you were, then you'd want to replace `-b dunedaq-v2.1.0` with `-b <branch you want to work on>`.
...
```sh
dbt-setup-build-environment  # Only needs to be done once in a given shell; establishes python virtual environment
...
* [[Creating a new package under v2.1.0|Creating a new package under v2.1.0]]
