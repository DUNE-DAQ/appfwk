...
template <class T> DequeBuffer<T>::DequeBuffer() { Configure(); }

template <class T> void DequeBuffer<T>::Configure() {
...
template <class T> int DequeBuffer<T>::push(T &&obj) {
  int n_retries = 0;
  while (full()) {
...
template <class T> T DequeBuffer<T>::pop() {
  int n_retries = 0;
  while (empty()) {
